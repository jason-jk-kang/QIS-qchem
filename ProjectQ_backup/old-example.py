...
for point in range(1, n_points + 1):
    ...
    # exponentiate translates the OpenFermion qubit operator to a pyQuil PauliSum which, in turn, is generated into a circuit that can be simulated with the Forest-qvm or associated QVMs.
    pyquil_program = exponentiate(qubit_hamiltonian)
    qvm = QVMConnection()
    wf = qvm.wavefunction(pyquil_program)
    print('The {} has wavefunction amplitudes of', end=''.format(molecule.name))
    print(wf.amplitudes)
    ...
    pyquil_program_rotated = exponentiate(qubit_hamiltonian)
    print(pyquil_program_rotated)
